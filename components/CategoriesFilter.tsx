    ...
    return (
        <div className="flex gap-2 mt-4 justify-center flex-wrap">
            {categories.map((category, index) => (
                <span onClick={() => changeCategory(category.name)} key={index} className={`flex border ${currentCategory === category.name ? 'border-blue-1200 bg-slate-400/50' : 'border-slate-400'} bg-black px-3 hover:bg-slate-500/60 hover:cursor-pointer text-sm font-bold text-white rounded-full capitalize`}>
